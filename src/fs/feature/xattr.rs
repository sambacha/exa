...
fn get_secattr(lister: &lister::Lister, c_path: &std::ffi::CString) -> io::Result<Vec<Attribute>> {
    ...
    let c_attr_name = match CString::new(SELINUX_XATTR_NAME) {
        Ok(cstring) => cstring,
        Err(e) => {
...
pub fn list_attrs(lister: &lister::Lister, path: &Path) -> io::Result<Vec<Attribute>> {
    let c_path = match path.to_str().map(CString::new) {
        Some(cstring_result) => match cstring_result {
            Ok(cstring) => cstring,
            Err(e) => {
                return Err(io::Error::new(io::ErrorKind::Other, e));
            }
        ...
        None => {
            return Err(io::Error::new(io::ErrorKind::Other, "Error: path not convertible to string"));
        }
    };
    ...
    for attr_name in buf.split(|c| c == &0) {
        ...
        let c_attr_name = match CString::new(attr_name) {
            Ok(cstring) => cstring,
            Err(e) => {
                return Err(io::Error::new(io::ErrorKind::Other, e));
            }
